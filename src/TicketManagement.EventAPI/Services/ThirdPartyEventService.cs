...
namespace TicketManagement.EventAPI.Services
{
	...
	public class ThirdPartyEventService : IThirdPartyEventService
    {
        ...
        private readonly IVenueLayoutGettingService _venueLayoutService;
        private string _webRootPath;
        private string _lastMessage;

		/// <summary>
		/// Initializes a new instance of the <see cref="ThirdPartyEventService"/> class.
		/// Initializes services for event.
		/// </summary>
		/// <param name="eventService">Event service.</param>
		/// <param name="venueLayoutService">Service for getting venues and layouts.</param>
		public ThirdPartyEventService(IEventService eventService, IVenueLayoutGettingService venueLayoutService)
        {
        ...
        private async Task<List<ThirdPartyEvent>> ImportEvents(IEnumerable<ThirdPartyEvent> thirdPartyEvents)
        {
            ...
            {
                var venue = await _venueLayoutService.GetVenueAsync(item.VenueName);
                if (venue != null)
                {
                    var layouts = await _venueLayoutService.GetAllLayoutsAsync(venue.Id);
